...
type SiteRow={organizationId:string;heroTitle:string;heroSubtitle:string;primaryCta:string;appDownloadUrl:string|null;supportEmail:string|null;latestVersion:string;releaseNotes:string;maintenanceActive:boolean;maintenanceMessage:string|null;paymentMethods:unknown;faq:unknown;regionalSettings:unknown};
...
export class LocalizedSiteService{
  ...
  private policy(settings:unknown,region:MarketRegion):RegionPolicy{
    ...
    return{
      ...
      showDownload:value.showDownload!==false,
      showReviews:value.showReviews!==false,
      showPromoVideo:value.showPromoVideo!==false,
      forceCurrency:SUPPORTED_MARKET_CURRENCIES.includes(requested as MarketCurrency)?requested as MarketCurrency:undefined,
      heroTitle:typeof value.heroTitle==='string'&&value.heroTitle.trim()?value.heroTitle.trim():undefined,
      heroSubtitle:typeof value.heroSubtitle==='string'&&value.heroSubtitle.trim()?value.heroSubtitle.trim():undefined,
      primaryCta:typeof value.primaryCta==='string'&&value.primaryCta.trim()?value.primaryCta.trim():undefined,
      announcement:typeof value.announcement==='string'&&value.announcement.trim()?value.announcement.trim():undefined,
    };
  }
  async publicSite(country?:string|null,currency?:string|null){
    ...
    return{
      ...
      market,
      regionPolicy,
      supportedCurrencies:SUPPORTED_MARKET_CURRENCIES,
  ...
  normalizeRegionalSettings(value:unknown){
    const source=value&&typeof value==='object'&&!Array.isArray(value)?value as Record<string,unknown>:{};
    const output:Record<string,RegionPolicy>={};for(const region of REGION_KEYS)output[region]=this.policy(source,region);return output;
  }
}
