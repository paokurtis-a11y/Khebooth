...
type PlanRow={organizationId:string;code:string;name:string;priceMonthlyChf:number|null;active:boolean};
...
export class PromotionCheckoutService{
  ...
  private async plan(organizationId:string,code:string):Promise<PlanRow>{const rows=await this.prisma.$queryRaw<PlanRow[]>`SELECT "organizationId",code,name,"priceMonthlyChf",active FROM "SubscriptionPlanConfig" WHERE "organizationId"=${organizationId}::uuid AND code=${code} AND active=TRUE LIMIT 1`;if(!rows[0])throw new BadRequestException('Unknown subscription plan');return rows[0];}
  ...
  async checkout(payload:Record<string,unknown>){
    ...
    const campaign=await this.promotion(organizationId,plan.code);const discount=campaign?.discountPercent??0;const amount=Math.max(0,Math.round(plan.priceMonthlyChf*(100-discount)/100));
    ...
    params.set('success_url',`${webOrigin}/subscribe/success?session_id={CHECKOUT_SESSION_ID}`);params.set('cancel_url',`${webOrigin}/subscribe?plan=${encodeURIComponent(plan.code)}`);params.set('customer_email',email);params.set('metadata[clientId]',client.id);params.set('metadata[planCode]',plan.code);params.set('metadata[paymentMethod]',paymentMethod);params.set('metadata[amountCents]',String(amount));if(campaign)params.set('metadata[campaignId]',campaign.id);params.set('line_items[0][quantity]','1');params.set('line_items[0][price_data][currency]','chf');params.set('line_items[0][price_data][unit_amount]',String(amount));params.set('line_items[0][price_data][product_data][name]',`KHE Booth ${plan.name}${discount?` · Offre -${discount}%`:''}`);
    if(isTwint){params.set('mode','payment');params.append('payment_method_types[]','twint');params.set('metadata[billingMode]','twint_manual_renewal');}else{params.set('mode','subscription');params.append('payment_method_types[]','card');params.set('line_items[0][price_data][recurring][interval]','month');params.set('subscription_data[metadata][clientId]',client.id);params.set('subscription_data[metadata][planCode]',plan.code);params.set('subscription_data[metadata][amountCents]',String(amount));if(campaign)params.set('subscription_data[metadata][campaignId]',campaign.id);}
    ...
    await this.marketing.trackServer(organizationId,'CHECKOUT_STARTED',client.id,plan.code,amount,{paymentMethod,campaignId:campaign?.id??null,discountPercent:discount,autoRenew:!isTwint});
    return{checkoutUrl:data.url,sessionId:data.id,discountPercent:discount,automaticRenewal:!isTwint,twintManualRenewal:isTwint};
  }
}
