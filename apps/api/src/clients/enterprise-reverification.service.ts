...
export class EnterpriseReverificationService{
  ...
  private async issueLink(organizationId:string,clientId:string){const raw=randomBytes(32).toString('base64url');const expiresAt=new Date(Date.now()+TOKEN_TTL_MS);await this.prisma.$executeRaw`INSERT INTO "EnterpriseOnboardingToken" (id,"organizationId","clientId","tokenHash","expiresAt") VALUES (gen_random_uuid(),${organizationId}::uuid,${clientId}::uuid,${hash(raw)},${expiresAt})`;const origin=(process.env.WEB_ORIGIN||'https://khebooth.vercel.app').split(',')[0].trim().replace(/\/$/,'');return`${origin}/enterprise/reverification/${raw}`;}

  private async tokenContext(rawToken:string){const tokenHash=hash(String(rawToken||'').trim());const rows=await this.prisma.$queryRaw<any[]>`
    SELECT t."organizationId",t."clientId",t."expiresAt",t."revokedAt",c.name,c.email,c."subscriptionPlan",c."subscriptionStatus",c."paymentStatus",c."subscriptionEndsAt",
           o."verificationCycle",o."reverificationStatus",o."reverificationStartedAt",o."reverificationDueAt",o."nextDocumentVerificationAt",o."documentsVerifiedAt"
    ...
    const row=rows[0];if(!row||row.revokedAt||new Date(row.expiresAt).getTime()<Date.now())throw new UnauthorizedException('Lien de re-vérification invalide ou expiré.');return row;}

  async publicStatus(rawToken:string){const row=await this.tokenContext(rawToken);if(row.subscriptionPlan!=='ENTERPRISE')throw new ForbiddenException('Ce dossier n’est plus associé à une offre Enterprise.');const expired=Boolean(row.subscriptionEndsAt&&new Date(row.subscriptionEndsAt).getTime()<=Date.now());const docs=await this.prisma.$queryRaw<any[]>`
    SELECT id,"documentType","originalFileName",status,"uploadedAt","verifiedAt","rejectionReason","verificationCycle"
    FROM "EnterpriseVerificationDocument" WHERE "clientId"=${row.clientId}::uuid AND "deletedAt" IS NULL AND "verificationCycle"=${Number(row.verificationCycle||0)}
      AND "documentType" IN ('IDENTITY_CARD','PASSPORT','PROOF_OF_ADDRESS') ORDER BY "uploadedAt" DESC`;
    const identity=docs.find((d:any)=>(d.documentType==='IDENTITY_CARD'||d.documentType==='PASSPORT')&&d.status!=='REJECTED')??null;const address=docs.find((d:any)=>d.documentType==='PROOF_OF_ADDRESS'&&d.status!=='REJECTED')??null;return{client:{name:row.name,email:row.email},subscription:{active:row.subscriptionStatus==='ACTIVE'&&!expired&&row.paymentStatus==='PAID',plan:row.subscriptionPlan,status:row.subscriptionStatus,paymentStatus:row.paymentStatus,endsAt:row.subscriptionEndsAt},cycle:Number(row.verificationCycle||0),status:row.reverificationStatus,dueAt:row.reverificationDueAt||row.nextDocumentVerificationAt,lastVerifiedAt:row.documentsVerifiedAt,documents:docs,requirements:{identity:{received:Boolean(identity),status:identity?.status??'MISSING'},proofOfAddress:{received:Boolean(address),status:address?.status??'MISSING'}},humanReviewRequired:true,contractResignRequired:false,message:row.reverificationStatus==='VERIFIED'?'Votre vérification annuelle est à jour.':row.reverificationStatus==='CHANGES_REQUESTED'?'Un document doit être remplacé. Consultez le motif puis déposez une nouvelle version.':'Actualisez uniquement votre pièce d’identité/passeport et votre preuve de domicile. Le reste de votre dossier reste enregistré.'};}
