...
export class LiveShiftController{
  constructor(private readonly live:LiveShiftService,private readonly handover:ShiftHandoverService){}
  ...
  @Roles(UserRole.OWNER,UserRole.ADMIN,UserRole.OPERATOR) @Post('pulse') async pulse(@CurrentUser() user:AuthenticatedUser){const live=await this.live.pulse(user);const handover=await this.handover.pulse(user).catch(()=>({prepared:0,conversations:0}));return{...live,handover};}
}
