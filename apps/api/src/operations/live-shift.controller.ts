...
export class LiveShiftController{
  constructor(private readonly live:LiveShiftService,private readonly handover:ShiftHandoverService,private readonly brief:ShiftBriefService,private readonly rescue:SlaRescueService){}
  ...
  @Roles(UserRole.OWNER,UserRole.ADMIN,UserRole.OPERATOR) @Post('pulse') async pulse(@CurrentUser() user:AuthenticatedUser){const live=await this.live.pulse(user);const brief=await this.brief.pulse(user).catch(()=>({prepared:0}));const rescue=await this.rescue.pulse(user).catch(()=>({created:0,refreshed:0}));const handover=await this.handover.pulse(user).catch(()=>({prepared:0,conversations:0}));return{...live,brief,rescue,handover};}
}
