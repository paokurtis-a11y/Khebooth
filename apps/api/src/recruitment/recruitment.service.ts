...
export class RecruitmentService {
  ...
  async confirmDocument(rawToken:string,payload:Record<string,unknown>){
    ...
    if(blobUrl){
      try{const parsed=new URL(blobUrl);if(parsed.protocol!=='https:'||!parsed.hostname.endsWith('.blob.vercel-storage.com'))throw new Error('invalid blob URL');blobLookup=blobUrl;}catch{throw new BadRequestException('Référence de stockage invalide');}
    }
    try{const blob=await head(blobLookup);const matches={pathname:blob.pathname===pathname||blob.pathname.endsWith(`/${pathname}`),size:blob.size===byteSize,contentType:blob.contentType===contentType};if(!matches.pathname||!matches.size||!matches.contentType){console.warn('[recruitment] document verification mismatch',matches);throw new BadRequestException('Le document reçu ne correspond pas au fichier annoncé');}}catch(error){if(error instanceof BadRequestException)throw error;console.error('[recruitment] document confirmation failed',error instanceof Error?error.message:'blob verification failed');throw new BadRequestException('Document introuvable après le téléversement');}
