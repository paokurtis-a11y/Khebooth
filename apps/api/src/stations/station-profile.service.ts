import { Injectable } from '@nestjs/common';
import { Prisma } from '@prisma/client';
import { PrismaService } from '../prisma/prisma.service';
import type { AuthenticatedStation } from './station-auth.types';
import type { UpdateStationProfileDto } from './dto/update-station-profile.dto';

interface ProfileRow {
  organizationId: string;
  firstName: string;
  lastName: string;
  displayName: string;
  company: string;
  role: string;
  email: string;
  phone: string;
  city: string;
  country: string;
  bio: string;
  updatedAt: Date;
}

const EMPTY = {
  firstName: '', lastName: '', displayName: '', company: '', role: '', email: '', phone: '', city: '', country: '', bio: '',
};

@Injectable()
export class StationProfileService {
  constructor(private readonly prisma: PrismaService) {}

  async get(station: AuthenticatedStation) {
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO "OrganizationProfile" ("organizationId")
      VALUES (${station.organizationId}::uuid)
      ON CONFLICT ("organizationId") DO NOTHING
    `);
    const rows = await this.prisma.$queryRaw<ProfileRow[]>(Prisma.sql`
      SELECT "organizationId", "firstName", "lastName", "displayName", "company", "role", "email", "phone", "city", "country", "bio", "updatedAt"
      FROM "OrganizationProfile"
      WHERE "organizationId" = ${station.organizationId}::uuid
      LIMIT 1
    `);
    return rows[0] ?? { organizationId: station.organizationId, ...EMPTY, updatedAt: new Date() };
  }

  async update(station: AuthenticatedStation, dto: UpdateStationProfileDto) {
    const current = await this.get(station);
    const next = {
      firstName: dto.firstName ?? current.firstName,
      lastName: dto.lastName ?? current.lastName,
      displayName: dto.displayName ?? current.displayName,
      company: dto.company ?? current.company,
      role: dto.role ?? current.role,
      email: dto.email ?? current.email,
      phone: dto.phone ?? current.phone,
      city: dto.city ?? current.city,
      country: dto.country ?? current.country,
      bio: dto.bio ?? current.bio,
    };

    const rows = await this.prisma.$queryRaw<ProfileRow[]>(Prisma.sql`
      INSERT INTO "OrganizationProfile" (
        "organizationId", "firstName", "lastName", "displayName", "company", "role", "email", "phone", "city", "country", "bio", "updatedAt"
      ) VALUES (
        ${station.organizationId}::uuid, ${next.firstName}, ${next.lastName}, ${next.displayName}, ${next.company}, ${next.role}, ${next.email}, ${next.phone}, ${next.city}, ${next.country}, ${next.bio}, CURRENT_TIMESTAMP
      )
      ON CONFLICT ("organizationId") DO UPDATE SET
        "firstName" = EXCLUDED."firstName",
        "lastName" = EXCLUDED."lastName",
        "displayName" = EXCLUDED."displayName",
        "company" = EXCLUDED."company",
        "role" = EXCLUDED."role",
        "email" = EXCLUDED."email",
        "phone" = EXCLUDED."phone",
        "city" = EXCLUDED."city",
        "country" = EXCLUDED."country",
        "bio" = EXCLUDED."bio",
        "updatedAt" = CURRENT_TIMESTAMP
      RETURNING "organizationId", "firstName", "lastName", "displayName", "company", "role", "email", "phone", "city", "country", "bio", "updatedAt"
    `);
    return rows[0];
  }
}
