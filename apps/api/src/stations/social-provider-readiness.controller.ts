...
export class SocialProviderReadinessController {
  constructor(private readonly sharingBusiness: SharingBusinessService, private readonly connections: SocialProviderConnectionV2Service, private readonly metaBusiness: MetaBusinessOAuthService, private readonly metaCallback: MetaBusinessCallbackService) {}
  ...
  @UseGuards(StationAuthGuard) @Post('oauth/:provider/start') start(@CurrentStation() station:AuthenticatedStation,@Param('provider') provider:string){const parsed=this.provider(provider);if(parsed==='FACEBOOK'||parsed==='INSTAGRAM')return this.metaBusiness.start(station,parsed);return this.connections.startOAuth(station,parsed);}
  @Get('oauth/:provider/callback') callback(@Param('provider') provider:string,@Query('code') code?:string,@Query('state') state?:string,@Query('error') error?:string,@Query('error_description') errorDescription?:string){if(error)throw new BadRequestException(`Autorisation refusée par le fournisseur : ${errorDescription||error}`);const parsed=this.provider(provider);if(parsed==='FACEBOOK'||parsed==='INSTAGRAM')return this.metaCallback.callback(parsed,String(code??''),String(state??''));return this.connections.oauthCallback(parsed,String(code??''),String(state??''));}
