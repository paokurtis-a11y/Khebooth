ALTER TABLE "User"
  ...
  ADD COLUMN IF NOT EXISTS "notificationPreferences" JSONB NOT NULL DEFAULT '{"enabled":true,"soundEnabled":true,"sound":"khe_chime","soundVolume":70,"vibrationEnabled":true,"vibrationMode":"double","vibrationIntensity":"medium"}'::jsonb;
...
CREATE INDEX IF NOT EXISTS "TeamInvitation_organizationId_email_idx" ON "TeamInvitation"("organizationId", "email");
CREATE INDEX IF NOT EXISTS "TeamInvitation_expiresAt_idx" ON "TeamInvitation"("expiresAt");

CREATE TABLE IF NOT EXISTS "ClientWorkspaceState" (
  "clientId" UUID PRIMARY KEY REFERENCES "Client"("id") ON DELETE CASCADE,
  "organizationId" UUID NOT NULL REFERENCES "Organization"("id") ON DELETE CASCADE,
  "selectedEventId" UUID REFERENCES "Event"("id") ON DELETE SET NULL,
  "designConfig" JSONB NOT NULL DEFAULT '{}'::jsonb,
  "designReadyAt" TIMESTAMP(3),
  "updatedAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS "ClientWorkspaceState_org_event_idx" ON "ClientWorkspaceState"("organizationId", "selectedEventId");
