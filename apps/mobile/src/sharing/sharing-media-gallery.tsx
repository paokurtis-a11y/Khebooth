...
import { deleteSharingTrashManyPermanently, deleteSharingTrashPermanently, emptySharingTrash, listSharingTrash, restoreSharingMedia, trashSharingMediaMany, type SharingTrashItem } from './sharing-trash-client';
...
export function SharingMediaGallery({ eventId, eventName, api, stationToken, store }: SharingMediaGalleryProps) {
  ...
  async function loadTrash(): Promise<void> {
    ...
    try { const items=await listSharingTrash(stationToken);setTrashItems(items);setTrashSelectedIds(current=>new Set([...current].filter(id=>items.some(item=>item.id===id)))); }
  ...
  function removeFromActive(ids:string[]):void {
    ...
    setUnmutedVideoIds((current)=>new Set([...current].filter((id)=>!idSet.has(id))));
  ...
  function toggleTrashSelection(id:string){setTrashSelectedIds(current=>{const next=new Set(current);if(next.has(id))next.delete(id);else next.add(id);return next;});}
  function confirmPermanentTrash(ids:string[]):void{if(!ids.length)return;Alert.alert('Supprimer définitivement ?',`${ids.length} média${ids.length===1?' sera':'s seront'} effacé${ids.length===1?'':'s'} du Cloud KHE. Cette action est irréversible.`,[{text:'Annuler',style:'cancel'},{text:'Supprimer définitivement',style:'destructive',onPress:()=>void(async()=>{setBusyId('trash-delete');try{const result=ids.length===1?await deleteSharingTrashPermanently(stationToken,ids[0]).then(()=>({deleted:1,failed:[]})):await deleteSharingTrashManyPermanently(stationToken,ids);setTrashSelectedIds(new Set());setMessage(result.failed.length?`${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'} ; ${result.failed.length} échec${result.failed.length===1?'':'s'} à réessayer.`:`✓ ${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'} définitivement.`);await loadTrash();}catch(error){setMessage(error instanceof Error?error.message:'Suppression définitive impossible.');}finally{setBusyId(null);}})()}]);}
  function confirmEmptyTrash():void{if(!trashItems.length)return;Alert.alert('Vider toute la corbeille ?',`${trashItems.length} média${trashItems.length===1?' sera':'s seront'} supprimé${trashItems.length===1?'':'s'} définitivement.`,[{text:'Annuler',style:'cancel'},{text:'Vider la corbeille',style:'destructive',onPress:()=>void(async()=>{setBusyId('trash-empty');try{const result=await emptySharingTrash(stationToken);setTrashSelectedIds(new Set());setMessage(result.failed.length?`${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'} ; ${result.failed.length} échec${result.failed.length===1?'':'s'}.`:'✓ Corbeille vidée définitivement.');await loadTrash();}catch(error){setMessage(error instanceof Error?error.message:'Impossible de vider la corbeille.');}finally{setBusyId(null);}})()}]);}
  ...
  return (
    <View style={styles.container}>
      ...
      {trashOpen&&businessEnabled?<View style={styles.trashPanel}><View style={styles.trashHeading}><View><Text style={styles.eyebrow}>BUSINESS • CORBEILLE</Text><Text style={styles.trashTitle}>Médias supprimés</Text></View><Text style={styles.trashHint}>Conservation automatique : 30 jours</Text></View>{trashItems.length?<View style={styles.trashToolbar}><Pressable style={styles.trashToolButton} onPress={()=>setTrashSelectedIds(trashSelectedIds.size===trashItems.length?new Set():new Set(trashItems.map(item=>item.id)))}><Text style={styles.trashToolText}>{trashSelectedIds.size===trashItems.length?'Tout désélectionner':'Tout sélectionner'}</Text></Pressable><Pressable disabled={!trashSelectedIds.size||Boolean(busyId)} style={[styles.trashDeleteButton,!trashSelectedIds.size&&styles.disabled]} onPress={()=>confirmPermanentTrash([...trashSelectedIds])}><Text style={styles.trashDeleteText}>Supprimer ({trashSelectedIds.size})</Text></Pressable><Pressable disabled={Boolean(busyId)} style={styles.trashDeleteButton} onPress={confirmEmptyTrash}><Text style={styles.trashDeleteText}>Tout vider</Text></Pressable></View>:null}{trashItems.length===0?<Text style={styles.help}>La corbeille est vide.</Text>:trashItems.map((item)=>{const selected=trashSelectedIds.has(item.id);return <View key={item.id} style={[styles.trashRow,selected&&styles.trashRowSelected]}><Pressable accessibilityRole="checkbox" accessibilityState={{checked:selected}} style={[styles.trashCheck,selected&&styles.trashCheckActive]} onPress={()=>toggleTrashSelection(item.id)}><Text style={styles.trashCheckText}>{selected?'✓':''}</Text></Pressable><Pressable style={{flex:1}} onPress={()=>toggleTrashSelection(item.id)}><Text style={styles.trashName}>{item.displayName||'Moment KHE'}</Text><Text style={styles.help}>{item.mimeType.startsWith('image/')?'PHOTO':'VIDÉO'} • {sizeLabel(item.byteSize)} • encore {remainingDays(item)} jour{remainingDays(item)===1?'':'s'}</Text></Pressable><View style={styles.trashRowActions}><Pressable disabled={Boolean(busyId)} style={styles.restoreButton} onPress={()=>void restoreTrash(item)}><Text style={styles.restoreText}>Restaurer</Text></Pressable><Pressable disabled={Boolean(busyId)} style={styles.permanentButton} onPress={()=>confirmPermanentTrash([item.id])}><Text style={styles.permanentText}>Supprimer</Text></Pressable></View></View>;})}</View>:null}
            ...
            return (
              <View key={item.id} style={[styles.mediaCard,{ width: cardWidth },selected&&styles.mediaCardSelected]}>
                ...
                <View style={[styles.previewFrame, { height: previewHeight(item.id) }]}>
                  <SharingMediaPreview uri={localUris[item.id] ?? null} mimeType={item.mimeType} mediaFit={businessEnabled ? businessSettings.mediaFit : 'COVER'} active={activeVideoId===item.id} muted={!unmutedVideoIds.has(item.id)} onActivate={()=>setActiveVideoId(item.id)} onToggleMuted={()=>setUnmutedVideoIds((current)=>{const next=new Set(current);if(next.has(item.id))next.delete(item.id);else next.add(item.id);return next;})} onAspectRatio={(ratio) => rememberAspect(item.id, ratio)} />
...
const styles = StyleSheet.create({
  ...
  trashPanel:{backgroundColor:'#171717',borderRadius:16,padding:13,gap:9,borderWidth:1,borderColor:'#3b3320'},trashHeading:{flexDirection:'row',justifyContent:'space-between',alignItems:'center',gap:8},trashTitle:{color:'#fff',fontSize:17,fontWeight:'900'},trashHint:{color:'#d2ad4f',fontSize:9,fontWeight:'900'},trashToolbar:{flexDirection:'row',flexWrap:'wrap',gap:6},trashToolButton:{borderWidth:1,borderColor:'#d2ad4f',borderRadius:9,paddingHorizontal:9,paddingVertical:7},trashToolText:{color:'#d2ad4f',fontSize:9,fontWeight:'900'},trashDeleteButton:{borderWidth:1,borderColor:'#d75a62',borderRadius:9,paddingHorizontal:9,paddingVertical:7},trashDeleteText:{color:'#f1848b',fontSize:9,fontWeight:'900'},trashRow:{backgroundColor:'#222',borderRadius:12,padding:10,flexDirection:'row',alignItems:'center',gap:8},trashRowSelected:{borderWidth:1,borderColor:'#d2ad4f'},trashCheck:{width:24,height:24,borderRadius:7,borderWidth:1,borderColor:'#777',alignItems:'center',justifyContent:'center'},trashCheckActive:{backgroundColor:'#d2ad4f',borderColor:'#d2ad4f'},trashCheckText:{color:'#111',fontWeight:'900'},trashName:{color:'#fff',fontSize:11,fontWeight:'900'},trashRowActions:{gap:5},restoreButton:{borderWidth:1,borderColor:'#d2ad4f',borderRadius:9,paddingHorizontal:10,paddingVertical:8},restoreText:{color:'#d2ad4f',fontSize:9,fontWeight:'900'},permanentButton:{borderWidth:1,borderColor:'#d75a62',borderRadius:9,paddingHorizontal:10,paddingVertical:8},permanentText:{color:'#f1848b',fontSize:9,fontWeight:'900'},
