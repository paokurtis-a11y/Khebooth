...
export function SharingTrashPanel({api,stationToken}:{api:StationExperienceApi;stationToken:string}){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      setTrash(trashItems);
      setTrashSelected(current=>current.filter(id=>trashItems.some(item=>item.id===id)));
      setLocked(false);
    }catch(error){
  ...
  const allTrashSelected=useMemo(()=>trash.length>0&&trashSelected.length===trash.length,[trash.length,trashSelected.length]);

  function toggle(id:string){setSelected(current=>current.includes(id)?current.filter(value=>value!==id):[...current,id]);}
  function toggleTrash(id:string){setTrashSelected(current=>current.includes(id)?current.filter(value=>value!==id):[...current,id]);}
  ...
  async function permanentlyDelete(ids:string[]){if(!ids.length)return;setBusy(true);setMessage('');try{const result=ids.length===1?await deleteSharingTrashPermanently(stationToken,ids[0]).then(()=>({deleted:1,failed:[]})):await deleteSharingTrashManyPermanently(stationToken,ids);setTrashSelected([]);setMessage(result.failed.length?`${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'} définitivement. ${result.failed.length} échec${result.failed.length===1?'':'s'} à réessayer.`:`✓ ${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'} définitivement.`);await load();}catch(error){setMessage(error instanceof Error?error.message:'Suppression définitive impossible.');}finally{setBusy(false);}}
  function confirmPermanent(ids:string[]){if(!ids.length)return;Alert.alert('Supprimer définitivement ?',`${ids.length} média${ids.length===1?' sera':'s seront'} effacé${ids.length===1?'':'s'} du Cloud KHE. Cette action est irréversible.`,[{text:'Annuler',style:'cancel'},{text:'Supprimer définitivement',style:'destructive',onPress:()=>void permanentlyDelete(ids)}]);}
  function confirmEmpty(){if(!trash.length)return;Alert.alert('Vider toute la corbeille ?',`${trash.length} média${trash.length===1?' sera':'s seront'} supprimé${trash.length===1?'':'s'} définitivement. Cette action est irréversible.`,[{text:'Annuler',style:'cancel'},{text:'Vider la corbeille',style:'destructive',onPress:()=>void(async()=>{setBusy(true);setMessage('');try{const result=await emptySharingTrash(stationToken);setTrashSelected([]);setMessage(result.failed.length?`${result.deleted} média${result.deleted===1?'':'s'} supprimé${result.deleted===1?'':'s'}, ${result.failed.length} échec${result.failed.length===1?'':'s'} à réessayer.`:'✓ Corbeille vidée définitivement.');await load();}catch(error){setMessage(error instanceof Error?error.message:'Impossible de vider la corbeille.');}finally{setBusy(false);}})()}]);}
    ...
    {open?<View style={styles.body}>
      ...
      {!locked&&!loading?<>
        ...
        <View style={styles.section}>
          ...
          {trash.length?<View style={styles.toolbar}><Pressable disabled={busy} style={styles.secondary} onPress={()=>setTrashSelected(allTrashSelected?[]:trash.map(item=>item.id))}><Text style={styles.secondaryText}>{allTrashSelected?'Tout désélectionner':'Tout sélectionner'}</Text></Pressable><Pressable disabled={!trashSelected.length||busy} style={[styles.danger,!trashSelected.length&&styles.disabled]} onPress={()=>confirmPermanent(trashSelected)}><Text style={styles.dangerText}>Supprimer ({trashSelected.length})</Text></Pressable><Pressable disabled={busy} style={styles.emptyTrash} onPress={confirmEmpty}><Text style={styles.emptyTrashText}>Vider la corbeille</Text></Pressable></View>:null}
          {trash.length?trash.map((item,index)=>{
            const remaining=daysRemaining(item.trashExpiresAt);
            const checked=trashSelected.includes(item.id);return <View key={item.id} style={[styles.trashRow,checked&&styles.rowSelected]}>
              <Pressable accessibilityRole="checkbox" accessibilityState={{checked}} style={[styles.check,checked&&styles.checkActive]} onPress={()=>toggleTrash(item.id)}><Text style={styles.checkText}>{checked?'✓':''}</Text></Pressable>
              <Pressable style={{flex:1}} onPress={()=>toggleTrash(item.id)}><Text style={styles.rowTitle}>{item.displayName||`KHE ${item.mimeType.startsWith('video/')?'Vidéo':'Photo'} ${String(index+1).padStart(3,'0')}`}</Text><Text style={styles.meta}>Suppression définitive dans {remaining} jour{remaining===1?'':'s'} • {new Date(item.trashExpiresAt).toLocaleDateString()}</Text></Pressable>
              <View style={styles.trashActions}><Pressable disabled={busy} style={styles.restore} onPress={()=>void restore(item)}><Text style={styles.restoreText}>Restaurer</Text></Pressable><Pressable disabled={busy} style={styles.deleteForever} onPress={()=>confirmPermanent([item.id])}><Text style={styles.deleteForeverText}>Supprimer</Text></Pressable></View>
            </View>;
...
const styles=StyleSheet.create({
  ...
  trashRow:{flexDirection:'row',alignItems:'center',gap:8,backgroundColor:'#18181d',borderRadius:12,padding:10},trashActions:{gap:5},restore:{borderWidth:1,borderColor:'#64bf82',borderRadius:9,paddingHorizontal:9,paddingVertical:7},restoreText:{color:'#79d59d',fontSize:9,fontWeight:'900'},deleteForever:{borderWidth:1,borderColor:'#b94650',borderRadius:9,paddingHorizontal:9,paddingVertical:7},deleteForeverText:{color:'#ef858d',fontSize:9,fontWeight:'900'},emptyTrash:{borderWidth:1,borderColor:'#b94650',borderRadius:10,paddingHorizontal:10,paddingVertical:8},emptyTrashText:{color:'#ef858d',fontSize:9,fontWeight:'900'},empty:{color:'#8c8c94',fontSize:10,paddingVertical:5},locked:{backgroundColor:'#1c1c21',borderRadius:12,padding:12,gap:4},lockedTitle:{color:'#d2ad4f',fontWeight:'900'},message:{color:'#ddca98',fontSize:10,lineHeight:15,fontWeight:'700'},
});
