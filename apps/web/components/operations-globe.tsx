...
type GlobeOverview = { generatedAt: string; mode: Mode; window: WindowKey; capabilities: { canViewAll: boolean; managedAccount: boolean; accountPlan: string | null; contactScope: string }; clients: ClientPoint[]; relations: RelationRecord[]; growth: { enabled: boolean; disabledReason?: string | null; geographies: Geo[]; summary: { visits: number; visitors: number; planSelections: number; checkouts: number; conversions: number } } };
...
type Selected = { key: string; title: string; subtitle: string; details: string[]; contacts?: ContactAction[] } | null;
...
export function OperationsGlobe({ agents }: { agents: AgentPoint[] }) {
  ...
  useEffect(() => { if (isManagedAccount && (mode === 'growth' || mode === 'all')) setMode('agents'); }, [isManagedAccount, mode]);
  useEffect(() => {
  ...
  const modeOptions: Array<[Mode, string]> = [
    ...
    ...(!isManagedAccount ? [['growth', t.growth] as [Mode, string]] : []),
  ...
  const selectAgent = (agent: AgentPoint, clusterSize: number, clusterNames: string[]) => {
    ...
    const details: NonNullable<Selected> = {
      ...
      details:clusterSize > 1 ? clusterNames : [
  ...
  const selectClient = (client: ClientPoint, clusterSize: number, clusterNames: string[]) => {
  ...
  return <div className="operations-globe" aria-busy={loading}>
    ...
    {selected?<aside className="globe-detail" aria-live="polite" aria-label={t.details}><div><strong>{selected.title}</strong><div className="detail-subtitle">{selected.subtitle}</div></div><div className="detail-lines">{selected.details.map((line,index)=><span key={`${line}-${index}`}>{line}</span>)}{selected.contacts?.length?<div className="contact-actions">{selected.contacts.map((contact)=><a key={`${contact.kind}:${contact.href}:${contact.label}`} href={contact.href} aria-label={`${contact.label} ${contact.value}`}><b>{contact.kind==='phone'?'☎':'✉'} {contact.label}</b><span>{contact.value}</span></a>)}</div>:null}</div><button type="button" onClick={()=>setSelected(null)} aria-label={t.close}>×</button></aside>:null}
