...
export function OperationsGlobe({ agents, expanded = false }: { agents: AgentPoint[]; expanded?: boolean }) {
  ...
  }, [agents,clients,growth,insights.agent,insights.client,insights.place,language,searchTerm,t.events,world]);
  ...
  const reliabilityTotal=mode==='agents'?filteredAgents.length:mode==='clients'?filteredClients.length:mode==='relations'?filteredRelations.length:mode==='growth'?filteredGrowth.length:filteredAgents.length+filteredClients.length+filteredGrowth.length;
  const reliabilityLocated=mode==='agents'?agentLocatedCount:mode==='clients'?clientLocatedCount:mode==='relations'?relationLocatedCount:mode==='growth'?growthLocatedCount:agentLocatedCount+clientLocatedCount+growthLocatedCount;
  ...
  const reliabilityRecent=mode==='agents'?recentAgentCount:mode==='clients'?recentClientCount:mode==='relations'?recentRelationCount:mode==='growth'?0:recentAgentCount+recentClientCount;
  ...
  const selectCountry = (feature: Country) => {
    ...
    const details = { key:`country:${code}`,title,subtitle:code === ownerCountryCode ? t.ownerCurrent : code,details:[`${insights.reliability}: ${insights.aggregated}`,`${countryAgents} ${t.agents}`,`${countryClients} ${t.clients}`,`${countryVisitors} ${t.visitor}`] };
    setSelected(details);
