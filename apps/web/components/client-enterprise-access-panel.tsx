...
const onboardingLabels:Record<string,string>={PAYMENT_PENDING:'Paiement en attente',FORM_AVAILABLE:'Formulaire disponible',FORM_SUBMITTED:'Formulaire envoyé',IDENTITY_PENDING:'Justificatifs requis',UNDER_REVIEW:'Vérification KHE en cours',CHANGES_REQUESTED:'Complément demandé',VERIFIED:'Vérifié par agent KHE',APPROVED:'Approuvé par OWNER',REJECTED:'Dossier refusé'};

export function ClientEnterpriseAccessPanel({clientId,clientName,plan}:{clientId:string;clientName:string;plan:string}){
  const[report,setReport]=useState<EnterpriseReport|null>(null);const[loading,setLoading]=useState(false);const[busy,setBusy]=useState(false);const[message,setMessage]=useState('');const[error,setError]=useState('');
  ...
  async function setAccess(enabled:boolean){if(enabled&&report?.onboardingStatus!=='APPROVED'){setError('Le dossier Enterprise doit être vérifié par un agent KHE puis approuvé par le OWNER avant activation.');return;}const action=enabled?'activer':'désactiver';if(!window.confirm(`${action[0].toUpperCase()+action.slice(1)} l’accès KHE BOOTH Enterprise de « ${clientName} » ?`))return;setBusy(true);setMessage('');setError('');try{const next=await apiRequest<EnterpriseReport>(`/clients/${clientId}/enterprise-access`,{method:'POST',body:JSON.stringify({enabled})});setReport(next);setMessage(enabled?'✓ Accès Enterprise activé. KHE a envoyé la procédure sécurisée de création/réinitialisation du mot de passe au client.':'✓ Accès Enterprise désactivé et sessions précédentes invalidées.');}catch(caught){setError(caught instanceof Error?caught.message:'Modification impossible.');}finally{setBusy(false);}}
  ...
  const canActivate=report?.onboardingStatus==='APPROVED';
  return <section className="card" style={{marginTop:16,border:'1px solid rgba(210,173,79,.45)',background:'linear-gradient(145deg,#11161d,#17140d)'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:14,flexWrap:'wrap'}}><div><div className="eyebrow" style={{color:'#d2ad4f'}}>PORTEFEUILLE CLIENT • ENTERPRISE</div><h2 style={{margin:'6px 0'}}>Accès KHE BOOTH</h2><p className="muted" style={{maxWidth:760}}>Tenant client isolé de la plateforme KHE principale. L’activation et la désactivation restent exclusivement sous contrôle du propriétaire KHE.</p></div><span style={{padding:'8px 12px',borderRadius:999,background:report?.accessEnabled?'#143621':'#3a2525',color:report?.accessEnabled?'#8fe0aa':'#ffaaaa',fontWeight:900,fontSize:12}}>{report?.accessEnabled?'ACCÈS ACTIF':'ACCÈS INACTIF'}</span></div>
    ...
    {report?<>
      <div style={{marginTop:14,padding:'12px 14px',borderRadius:12,border:`1px solid ${canActivate?'#2f6744':'#5d4b23'}`,background:canActivate?'#102619':'#201b0f'}}><strong>Contrôle préalable : {onboardingLabels[report.onboardingStatus]||report.onboardingStatus}</strong><p className="muted" style={{margin:'5px 0 0'}}>{canActivate?'Le dossier est approuvé. Le OWNER peut maintenant activer ou désactiver l’accès.':'L’activation reste verrouillée jusqu’à la validation des pièces, la vérification par un agent KHE et l’approbation finale du OWNER.'}</p></div>
      ...
      <div className="toolbar" style={{marginTop:18}}><button type="button" className={report.accessEnabled?'button danger':'button'} disabled={busy||(!report.accessEnabled&&!canActivate)} onClick={()=>void setAccess(!report.accessEnabled)}>{busy?'Traitement…':report.accessEnabled?'Désactiver l’accès Enterprise':canActivate?'Activer l’accès Enterprise':'Accès verrouillé · vérification requise'}</button><button type="button" className="button secondary" disabled={loading||busy} onClick={()=>void load()}>↻ Actualiser le rapport</button></div>
    </>:null}
  </section>;
}
