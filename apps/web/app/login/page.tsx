...
import { apiRequest, getAccessToken, getSessionUser, setAccessToken, setSessionUser } from '@/lib/api';

type LoginResponse={accessToken:string;user:{id:string;email:string;username?:string|null;role:string}};
function homeForRole(role?:string|null){return role==='OPERATOR'?'/agent':'/dashboard';}
...
export default function LoginPage(){
  ...
  useEffect(()=>{if(getAccessToken())router.replace(homeForRole(getSessionUser()?.role));},[router]);
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      setAccessToken(result.accessToken);setSessionUser(result.user);router.replace(homeForRole(result.user.role));
    }catch(caught){
