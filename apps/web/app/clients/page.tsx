...
import { ClientEnterpriseAccessPanel } from '@/components/client-enterprise-access-panel';
import { ClientEnterpriseOnboardingPanel } from '@/components/client-enterprise-onboarding-panel';
import { EnterpriseFormConfigPanel } from '@/components/enterprise-form-config-panel';
...
type ClientTab='portfolio'|'enterprise-settings';
...
export default function ClientsPage() {
  ...
  useEffect(() => {const role = getSessionUser()?.role;setCanDelete(role === 'OWNER' || role === 'ADMIN');setIsOwner(role === 'OWNER');loadClients();}, [loadClients]);
  ...
  function editClient(client: ClientItem) {setTab('portfolio');const fallback = client.name.trim().split(/\s+/);setEditingId(client.id);setFirstName(client.firstName ?? fallback[0] ?? '');setLastName(client.lastName ?? fallback.slice(1).join(' ') ?? '');setEmail(client.email ?? '');setPhone(client.phone ?? '');setCompanyName(client.companyName ?? '');setNotes(client.notes ?? '');setSubscriptionPlan(client.subscriptionPlan ?? 'DISCOVERY');setSubscriptionStatus(client.subscriptionStatus ?? 'PROSPECT');setPaymentStatus(client.paymentStatus ?? 'UNPAID');setSubscriptionEndsAt(client.subscriptionEndsAt ? client.subscriptionEndsAt.slice(0, 10) : '');setMessage('');setError('');window.scrollTo({top:0,behavior:'smooth'});}

  async function submitClient(event: FormEvent<HTMLFormElement>) {event.preventDefault();setSubmitting(true);setError('');setMessage('');try {const body = JSON.stringify({firstName:firstName.trim(),name:lastName.trim(),email:email.trim(),subscriptionPlan,subscriptionStatus,paymentStatus,...(subscriptionEndsAt ? { subscriptionEndsAt: `${subscriptionEndsAt}T23:59:59.000Z` } : {}),...(phone.trim() ? { phone: phone.trim() } : {}),...(companyName.trim() ? { companyName: companyName.trim() } : {}),...(notes.trim() ? { notes: notes.trim() } : {})});if (editingId) {await apiRequest<ClientItem>(`/clients/${editingId}`, { method: 'PATCH', body });setMessage(subscriptionPlan==='ENTERPRISE'&&paymentStatus==='PAID'?'Client mis à jour. Le paiement Enterprise ouvre le parcours d’identification, mais pas encore l’accès plateforme.':'Client et abonnement mis à jour.');} else {await apiRequest<ClientItem>('/clients', { method: 'POST', body });setMessage(subscriptionPlan==='ENTERPRISE'&&paymentStatus==='PAID'?'Client ajouté. Le parcours Enterprise est maintenant disponible pour vérification.':'Client ajouté.');}resetForm();loadClients();} catch (caught) {setError(caught instanceof Error ? caught.message : 'Enregistrement impossible');} finally {setSubmitting(false);}}
  ...
  const editingClient=editingId?clients.find((client)=>client.id===editingId)??null:null;

  return <PortalShell>
    <div className="header"><div><h1>Clients</h1><p>Profil synchronisé CAPTURE/SHARING, abonnement, vérification Enterprise, devis et contrôle d’accès dans une seule zone.</p></div></div>
    <div className="toolbar" style={{marginBottom:16}}><button className={tab==='portfolio'?'button':'button secondary'} type="button" onClick={()=>setTab('portfolio')}>Portefeuille clients</button>{isOwner?<button className={tab==='enterprise-settings'?'button':'button secondary'} type="button" onClick={()=>setTab('enterprise-settings')}>Formulaires & offres Enterprise</button>:null}</div>
    {error ? <p className="error">{error}</p> : null}{message ? <p className="success">{message}</p> : null}

    {tab==='enterprise-settings'&&isOwner?<EnterpriseFormConfigPanel/>:<div className="grid client-grid">
      <section className="card client-editor-card">
        ...
        <form className="form" onSubmit={submitClient}>
          ...
          <p className="muted" style={{ fontSize: 12 }}>* Nom, prénom et e-mail sont obligatoires. Pour Enterprise, un paiement « Payé » ouvre le formulaire d’identification ; l’accès à la plateforme reste bloqué jusqu’à la vérification agent puis l’approbation OWNER.</p>
        ...
        {editingClient&&canDelete?<ClientEnterpriseOnboardingPanel clientId={editingClient.id} plan={editingClient.subscriptionPlan??'DISCOVERY'} />:null}
        {editingClient&&isOwner?<ClientEnterpriseAccessPanel clientId={editingClient.id} clientName={editingClient.firstName?`${editingClient.firstName} ${editingClient.lastName??''}`.trim():editingClient.name} plan={editingClient.subscriptionPlan??'DISCOVERY'} />:null}
      </section>
