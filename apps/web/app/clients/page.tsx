...
export default function ClientsPage() {
  ...
  const [subscriptionPlan, setSubscriptionPlan] = useState<SubscriptionPlan>('DISCOVERY');
  const [subscriptionStatus, setSubscriptionStatus] = useState<SubscriptionStatus>('PROSPECT');
  ...
  function resetForm() {
    ...
    setSubscriptionStatus('PROSPECT');
  ...
  function editClient(client: ClientItem) {
    ...
    setSubscriptionPlan(client.subscriptionPlan ?? 'DISCOVERY');
    setSubscriptionStatus(client.subscriptionStatus ?? 'PROSPECT');
    setPaymentStatus(client.paymentStatus ?? 'UNPAID');
    setSubscriptionEndsAt(client.subscriptionEndsAt ? client.subscriptionEndsAt.slice(0, 10) : '');
    setMessage('');
    setError('');
  }
  ...
  async function submitClient(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const body = JSON.stringify({
        ...
        subscriptionStatus,
        paymentStatus,
        ...(subscriptionEndsAt ? { subscriptionEndsAt: `${subscriptionEndsAt}T23:59:59.000Z` } : {}),
      ...
      if (editingId) {
        ...
        setMessage('Client et abonnement mis à jour.');
      } else {
  ...
  return <PortalShell>
    <div className="header">
      <div>
        <h1>Clients</h1>
        <p>Identité, abonnement, paiement et progression commerciale dans une seule fiche.</p>
      </div>
    </div>
    ...
    <div className="grid client-grid">
      <section className="card">
        ...
        <form className="form" onSubmit={submitClient}>
          ...
          <div className="subscription-editor">
            <div>
              <div className="eyebrow">ABONNEMENT KHE BOOTH</div>
              <h3 style={{ margin: '5px 0 4px' }}>{subscriptionPlanLabel(subscriptionPlan)}</h3>
              <p className="muted" style={{ margin: 0, fontSize: 13 }}>Même catalogue que le site promotionnel public.</p>
            </div>
            <div className="field"><label htmlFor="subscriptionPlan">Niveau d’abonnement</label><select id="subscriptionPlan" value={subscriptionPlan} onChange={(e) => setSubscriptionPlan(e.target.value as SubscriptionPlan)}>{SUBSCRIPTION_CATALOG.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}{plan.priceMonthlyChf === null ? ' · Sur mesure' : ` · CHF ${plan.priceMonthlyChf}/mois`}</option>)}</select></div>
            <div className="field"><label htmlFor="subscriptionStatus">Niveau d’action / parcours</label><select id="subscriptionStatus" value={subscriptionStatus} onChange={(e) => setSubscriptionStatus(e.target.value as SubscriptionStatus)}>{SUBSCRIPTION_STATUSES.map((status) => <option key={status} value={status}>{subscriptionStatusLabels[status]}</option>)}</select></div>
            <div className="field"><label htmlFor="paymentStatus">Paiement</label><select id="paymentStatus" value={paymentStatus} onChange={(e) => setPaymentStatus(e.target.value as PaymentStatus)}>{PAYMENT_STATUSES.map((status) => <option key={status} value={status}>{paymentStatusLabels[status]}</option>)}</select></div>
            <div className="field"><label htmlFor="subscriptionEndsAt">Fin d’abonnement éventuelle</label><input id="subscriptionEndsAt" type="date" value={subscriptionEndsAt} onChange={(e) => setSubscriptionEndsAt(e.target.value)} /></div>
          </div>
      ...
      <section className="card client-table-card">
        {clients.length === 0 ? <div className="empty">Aucun client.</div> : <table className="table"><thead><tr><th>Client</th><th>E-mail</th><th>Abonnement</th><th>Niveau d’action</th><th>Paiement</th><th></th></tr></thead><tbody>
          {clients.map((client) => {
            const plan = client.subscriptionPlan ?? 'DISCOVERY';
            const status = client.subscriptionStatus ?? 'PROSPECT';
            const payment = client.paymentStatus ?? 'UNPAID';
            return <tr key={client.id}>
              <td><button className="link-button" type="button" onClick={() => editClient(client)}>{client.firstName ? `${client.firstName} ${client.lastName ?? ''}`.trim() : client.name}</button>{client.companyName ? <div className="muted" style={{ fontSize: 12, marginTop: 4 }}>{client.companyName}</div> : null}</td>
              <td>{client.email ?? 'À compléter'}</td>
              <td><span className={`subscription-pill subscription-${plan.toLowerCase()}`}>{subscriptionPlanLabel(plan)}</span></td>
              <td><span className={`journey-pill journey-${status.toLowerCase()}`}>{subscriptionStatusLabels[status]}</span></td>
              <td><span className={`payment-pill payment-${payment.toLowerCase()}`}>{paymentStatusLabels[payment]}</span></td>
